...
GEAR.indigo Biz の `/settings/api-tokens` で PAT を発行してください。
...
いずれの設定でも、`GIBIZ_API_URL`（デプロイ先のベース URL）と `GIBIZ_API_TOKEN`（PAT）を渡します。

#### Claude Code (CLI)

```bash
claude mcp add --transport stdio \
  --env GIBIZ_API_URL=https://<your-deployment> \
...
```json
{
  "mcpServers": {
    "gear-indigo-biz": {
      ...
      "env": {
        "GIBIZ_API_URL": "https://<your-deployment>",
...
| 変数名 | 必須 | 説明 |
|--------|------|------|
| `GIBIZ_API_URL` | ○ | デプロイ先のベース URL。`https://` は任意ホスト、`http://` は loopback (`localhost` / `127.0.0.1` / `::1`) のみ許可 |
